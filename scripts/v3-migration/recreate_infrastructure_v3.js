...
async function recreateCollection(name, fields, rules = {}) {
    try {
        ...
        console.error(`❌ Error creando '${name}'. Check error_log.json`);
        fs.writeFileSync(
            path.resolve(__dirname, 'reports/error_log.json'),
...
async function main() {
    ...
    for (const lvl of INITIAL_LEVELS) {
        ...
        } catch (e) { console.error("x"); }
    }
    console.log("\n");

    // LEDGER
    const usersCol = await pb.collections.getOne('users');
    console.log(`ℹ️ Users ID: ${usersCol.id}`);

    const ledgerFields = [
        { name: 'from_user', type: 'relation', required: true, options: { collectionId: usersCol.id, cascadeDelete: false, maxSelect: 1, displayFields: ['username'] } },
        { name: 'to_user', type: 'relation', required: false, options: { collectionId: usersCol.id, cascadeDelete: false, maxSelect: 1, displayFields: ['username'] } },
    ...
    // Simple rule first
    const ledgerRule = "@request.auth.id != ''";
