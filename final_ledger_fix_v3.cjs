...
function generateTxHash(prefix = 'ADJ') {
...
async function reconcile() {
    ...
    for (const [userId, s] of Object.entries(userStats)) {
        ...
        if (diff !== 0) {
            ...
            // Usamos tipos existentes para evitar errores de validación de schema
            const type = diff > 0 ? 'GIFT' : 'PURCHASE';
            const description = `AJUSTE CONTABLE: ${diff === -50 ? "Duplicado backfill" : (diff > 0 ? "Tokens legacy" : "Diferencia auditoría")}`;
            ...
            try {
                const data = {
                    ...
                    tx_hash: generateTxHash(diff > 0 ? 'FIX+' : 'FIX-')
                };
                ...
                console.log(`   ❌ ERROR en @${s.username}:`, err.message);
            }
        }
    }
