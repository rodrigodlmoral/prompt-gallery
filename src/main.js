...
const AdminModal = () => `
<div id="adminModal" class="modal-overlay" style="display:none;"><div class="modal-container" style="max-width:1000px; width:95%">
    <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:15px">
        <h2 style="color:gold; margin:0">👑 Panel de Administración</h2>
        <button class="btn-outline" style="padding:5px 15px" onclick="window.closeModals()">Cerrar</button>
    </div>
    ...
    <div id="adminUserEditBox" style="display:none; background:#111; border:1px solid gold; border-radius:12px; padding:15px; margin-bottom:20px; box-shadow:0 0 20px rgba(255,215,0,0.1)">
    ...
    <!-- Sección Usuarios -->
    <div id="adminSecUsers">
        <div class="admin-table-container">
            <table class="admin-table">
                <thead>
                    <tr>
                        <th onclick="window.toggleAdminSort('username')">Usuario</th>
                        <th onclick="window.toggleAdminSort('email')">Email</th>
                        <th onclick="window.toggleAdminSort('role')">Rol</th>
                        <th>Estado</th>
                        <th>Acciones</th>
                    </tr>
                </thead>
    ...
    <!-- Sección Contenido -->
    <div id="adminSecContent" style="display:none">
        <div class="admin-table-container">
            <table class="admin-table">
                <thead>
                    <tr>
    ...
    <!-- Sección Migración -->
    <div id="adminSecMigrate" style="margin-bottom:20px; display:none; text-align:center">
        <div style="background:#222; padding:20px; border-radius:10px; border:1px solid #444">
            <h3>♻️ Mover Imágenes a Cloudinary</h3>
            <p style="color:#aaa; max-width:500px; margin:0 auto 20px; font-size:0.9rem">
                Mueve imágenes de Supabase a Cloudinary para liberar espacio.
            </p>
            <div id="migrateStatus" style="font-size:1.1rem; font-weight:bold; margin-bottom:10px">Esperando...</div>
            <div id="migrateProgress" style="width:100%; height:8px; background:#444; border-radius:4px; overflow:hidden; margin-bottom:20px; display:none">
...
window.setAdminFilter = (char) => {
    // If clicking same char, toggle off (show all). Else set char.
    // If char is empty string, it means "ALL".
...
window.openAdmin = (tab = 'users') => {
    try {
        ...
        if (tab === 'users') {
            ...
            // 3. UI: Inject Alphabet Filter (if not exists)
            const secUsers = document.getElementById('adminSecUsers');
            if (secUsers && !document.getElementById('adminAlphabetFilter')) {
                const filterContainer = document.createElement('div');
                filterContainer.id = 'adminAlphabetFilter';
                filterContainer.style.cssText = 'margin-bottom:15px; display:flex; gap:4px; flex-wrap:wrap; justify-content:center';

                let html = `<button class="btn-sm ${!window.adminFilterChar ? 'active' : ''}" onclick="window.setAdminFilter('')" style="padding:2px 8px">ALL</button>`;
                const alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ".split("");
                alphabet.forEach(char => {
                    const isActive = window.adminFilterChar === char ? 'active' : '';
                    html += `<button class="btn-sm ${isActive}" onclick="window.setAdminFilter('${char}')" style="padding:2px 8px">${char}</button>`;
                });
                filterContainer.innerHTML = html;

                // Insert into DOM
                const tableWrapper = secUsers.querySelector('.table-container') || secUsers.querySelector('table');

                // Safe insertion: Check if tableWrapper is actually a direct child
                if (tableWrapper && tableWrapper.parentNode === secUsers) {
                    secUsers.insertBefore(filterContainer, tableWrapper);
                } else {
                    // Fallback: Just put it at the top of the section
                    secUsers.prepend(filterContainer);
                }
            } else if (document.getElementById('adminAlphabetFilter')) {
                // Update active buttons
                const container = document.getElementById('adminAlphabetFilter');
                Array.from(container.children).forEach(btn => {
                    const txt = btn.innerText;
                    if (txt === 'ALL') {
                        btn.className = `btn-sm ${!window.adminFilterChar ? 'active' : ''}`;
                    } else {
                        btn.className = `btn-sm ${window.adminFilterChar === txt ? 'active' : ''}`;
                    }
                });
            }

            // 4. UI: Refresh Headers (Split Columns)
            const thead = document.querySelector('#adminSecUsers thead tr');
            if (thead) {
                thead.innerHTML = `
                    <th onclick="window.toggleAdminSort('username')">
                        Usuario ${col === 'username' ? (dir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th onclick="window.toggleAdminSort('email')">
                        Email ${col === 'email' ? (dir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th onclick="window.toggleAdminSort('role')">
                        Rol ${col === 'role' ? (dir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <!-- SPLIT COLUMNS -->
                    <th onclick="window.toggleAdminSort('level')" style="width:80px">
                        Nivel ${col === 'level' ? (dir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th onclick="window.toggleAdminSort('tokens')" style="width:80px">
                        Bits ${col === 'tokens' ? (dir === 'asc' ? '↑' : '↓') : ''}
                    </th>
                    <th>Acciones</th>
                `;
            }

            console.log("Cargando", users.length, "usuarios en admin (Sorted by " + col + ")");
                ...
                tbody.innerHTML = users.map(u => `
                    <tr>
                        ...
                        <td style="text-align:center">
                            <span style="color:#aaa">Lvl ${u.level || 0}</span>
                        </td>
                        <td style="text-align:center">
                             <span style="color:#a29bfe; font-weight:bold">💎 ${u.tokens || 0}</span>
                        </td>
                        ...
                        <td>
                            <div style="display:flex; gap:5px">
                                ...
                                <button class="btn-outline" style="padding:4px 8px; font-size:0.7rem" onclick="window.setProfileView('${u.username}'); window.closeModals()">Ver</button>
            ...
            console.log("Cargando", prompts.length, "prompts en admin");
            const tbody = document.getElementById('adminPostList');
            if (tbody) {
                tbody.innerHTML = prompts.map(p => `
                    <tr style="border-bottom:1px solid #222">
                        <td style="padding:10px">
                            <img src="${p.type === 'sequence' ? p.content[0].image : p.image}" style="width:40px; height:40px; object-fit:cover; border-radius:4px; cursor:pointer" onclick="window.openDetail('${p.id}')">
                        </td>
                        <td>
        ...
        console.error("Error en openAdmin:", e);
        alert("Error abriendo el panel de admin. Revisa la consola.");
    }
};

window.doPromotePrompt = async (id) => {
    if (!confirm('¿Destacar este prompt por 1 semana? Costo: 50 PromptBits')) return;

    const result = await store.promotePrompt(id);
    if (result.success) {
        alert(result.msg);
        render();
    } else {
...
window.adminDeletePrompt = async (id) => {
    if (confirm("¿Estás seguro de eliminar este post como Administrador?")) {
        const res = await store.removePrompt(id);
        if (res.success) {
            alert("✅ Post eliminado");
            window.openAdmin('content');
            render();
        } else {
            alert("Error: " + res.msg);
        }
...
window.adminChangeRating = async (id, newRating) => {
    const p = store.prompts.find(x => x.id === id);
    if (!p) return;

    const res = await store.updatePrompt(id, { ...p, rating: newRating });
    if (res.success) {
        console.log("Rating actualizado por Admin");
        render(); // Update gallery in background
    } else {
        alert("Error cambiando rating: " + res.msg);
    }
};

window.adminDeleteUser = (id) => {
    if (confirm("¿Estás SEGURO de eliminar a este usuario y todos sus posts?")) {
        store.adminDeleteUser(id);
        window.openAdmin(); // Refresh list
        render(); // Refresh gallery
    }
};
...
window.adminOpenUserMgmt = (userId, username, level) => {
    currentEditingUserId = userId;

    const editBox = document.getElementById('adminUserEditBox');
    editBox.style.cssText = 'display: block !important; visibility: visible !important; opacity: 1 !important;';

    document.getElementById('editUserName').innerText = `Gestionar @${username}`;
    document.getElementById('editUserLevel').value = level;

    // Fix: usar querySelectorAll para el input duplicado
    const inputs = document.querySelectorAll('#giftBitsAmount');
    if (inputs.length > 0) inputs[inputs.length - 1].value = '';

    // Check current badge
    const u = store.getAllUsers().find(x => x.id === userId);
    const hasCreatorBadge = u?.badges?.some(b => b.type === 'creator_founder');
    document.getElementById('editUserBadge').value = hasCreatorBadge ? 'creator_founder' : 'none';

    // Scroll suavemente al panel
    setTimeout(() => editBox.scrollIntoView({ behavior: 'smooth', block: 'nearest' }), 100);
};

window.adminSubmitUserUpdate = async () => {
    if (!currentEditingUserId) return;
    const level = parseInt(document.getElementById('editUserLevel').value);
    const badgeType = document.getElementById('editUserBadge').value;

    const data = { level };
    if (badgeType === 'creator_founder') {
        data.badges = [{ type: 'creator_founder', label: 'CREADOR FUNDADOR' }];
    } else {
        data.badges = [];
    }

    const res = await store.adminUpdateUser(currentEditingUserId, data);
    if (res.success) {
        alert("✅ Cambios de nivel/medalla guardados correctamente.");
...
window.adminSubmitGift = async () => {
    ...
    // FIX: Hay IDs duplicados, usar querySelectorAll y tomar el último (el visible)
    const inputs = document.querySelectorAll('#giftBitsAmount');
...
window.togglePass = (id, btn) => {
    ...
    if (input.type === 'password') {
        input.type = 'text';
        btn.textContent = '🔒'; // Icon shows "Locked/Secret" when visible (or logic inverted: eye-slash). Let's use Open Eye / Slash.
        // Better: 
        // Password HIDDEN (dots) -> Show Eye -> Click -> Show Text -> Show Eye-Slash
        btn.textContent = '🙈';
    } else {
        input.type = 'password';
        btn.textContent = '👁️';
