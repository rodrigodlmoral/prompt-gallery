...
export const PromptsSemanal = ({ currentView, prompts }) => {
    ...
    if (!featuredList || featuredList.length === 0) {
        return `
        <div class="premium-spotlight-card" style="--glow-color: rgba(241, 196, 15, 0.15); margin-bottom: 25px; justify-content: center; text-align: center; min-height: 140px;">
            <div class="spotlight-glow"></div>
            <div class="spotlight-info" style="align-items: center;">
                <div class="spotlight-badge" style="--badge-color: #f1c40f">💎 TOP SEMANAL</div>
                <div style="color: #888; font-size: 0.95rem; margin: 8px 0;">No hay Boosts Semanales activos</div>
                <div style="color: #666; font-size: 0.8rem;">¡Destaca tu prompt por 7 días! Activa un Boost desde tu perfil.</div>
            </div>
        </div>`;
    }
    ...
    const dots = featuredList.map((_, i) =>
        `<span class="boost-dot ${i === 0 ? 'active' : ''}" data-dot="${i}" onclick="window.goBoostSlide('semanal', ${i})"></span>`
    ).join('');

    return `
    <div class="premium-spotlight-card boost-carousel-card" id="boost-semanal-carousel" style="--glow-color: rgba(241, 196, 15, 0.2); margin-bottom: 25px; position:relative;">
        <div class="spotlight-glow"></div>
        ${slides}
        <div class="boost-dots-container">
            ${dots}
        </div>
        <button class="boost-nav-btn boost-prev" onclick="event.stopPropagation(); window.navBoostSlide('semanal', -1)">‹</button>
        <button class="boost-nav-btn boost-next" onclick="event.stopPropagation(); window.navBoostSlide('semanal', 1)">›</button>
    </div>`;
};
